...
public class Receiver extends ReceiverBase {
     ...
     */
    private final Queue pduQueue = new Queue();
     ...
     * @throws PDUException              incorrect format of PDU
     * @throws TimeoutException          rest of PDU not received for too long time
        ...
        if (receiver) {
            ...
            synchronized (pduQueue) {
                if (expectedPDU == null) { // i.e. any pdu is acceptable
                    if (!pduQueue.isEmpty()) {
                        pdu = (PDU) pduQueue.dequeue();
                        debug.write(DRXTXD2, "dequeue:"+pdu.debugString());
                    }
                } else {
    ...
    public int getPduQueueSize() {
        return this.pduQueue.size();
    }
    /**
    ...
    protected void receiveAsync() {
        ...
        if (pdu != null) {
            ...
            if (getPduQueueSize() > 1)
                logger.info("[" + (systemId != null ? systemId : connection.getAddress()) + "-" + sessionId + "] PDU received: Seq:" + pdu.getSequenceNumber() + " PDU Buffer size:" + getPduQueueSize());
        }
    }
